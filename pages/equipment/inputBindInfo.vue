<template>
  <view class="content">
	  ...
	  <view
	    class="shadow"
	    @click="ShowPickerView=!ShowPickerView"
	    v-if="ShowPickerView"
	    ...
	    <view class="title" >
			<view class="leftIcon" @click="back">
			  ...
			  <view class="body-title">
				<text class="uni-nav-bar-text">{{$t('Equipment.InputBindInfo.bind-equipment')}}</text>
			  </view>
	    </view>
	  </view>
		...
		<view class="form-block">
			<form @submit="formSubmit" >
			  <view class="uni-form-item uni-column">
			    <uni-list-item :title="$t('Equipment.InputBindInfo.devices-code')" class="nav-top-line"></uni-list-item>
			    <input
			      class="uni-input"
			      :value="inputEquipmentCode"
			      name="equipmentID"
			      :placeholder="$t('Equipment.InputBindInfo.input-devices-code')"
			      :adjust-position="false"
			    />
			  </view>
			
			  <view class="uni-form-item uni-column">
			    <uni-list-item :title="$t('Equipment.InputBindInfo.devices-name')"></uni-list-item>
			    <input
			      class="uni-input"
			      :value="inputEquipmentName"
			      name="equipmentName"
			      :placeholder="$t('Equipment.InputBindInfo.input-devices-name')"
			      :adjust-position="false"
			    />
			  </view>
			
			  <view class="uni-list-cell">
			    <uni-list-item :title="$t('Equipment.InputBindInfo.sub-account')">
					<template v-slot:footer>
						<text class="selected-picker-view" @click="ShowPickerView=!ShowPickerView">{{productNameList[value[0]]}}&nbsp;&nbsp;
			    ...
			    <view class="uni-list-cell-db" v-if="ShowPickerView">
			      ...
			      >
			        <picker-view-column>
			          <view
			            class="item" v-for="(product, index) in productNameList" :key="index" @click="confirm">{{ product }}</view
			          >
			        </picker-view-column>
			      </picker-view>
			  ...
			  <view class="uni-btn-v">
			    <button
			      ...
			      class="uni-btn-add"
			    >
			      {{$t('Equipment.InputBindInfo.confirm')}}
			    </button>
			  </view>
			</form>
    ...
    >
      <view class="popup-content">
        <!-- 用v-if控制显示哪个信息 -->
        <text class="text" v-if="errorNum == 7">
          <uni-icons
            type="info-filled"
            class="icons"
            color="#f59a23"
          ></uni-icons
          >{{$t('Equipment.InputBindInfo.bind-fail')}}</text
        >
        <!-- <uni-icons type="info-filled" class="icons" color="#f59a23"></uni-icons>您输入的设备编号无效，请重新输入</text> -->
        <text class="text" v-if="errorBind">
          <uni-icons
            type="info-filled"
            class="icons"
            color="#f59a23"
          ></uni-icons
          >{{$t('Equipment.InputBindInfo.bind-error')}}</text
        >
        <text class="text" v-if="errorNum == 0">
          <uni-icons
            type="info-filled"
            class="icons"
            color="#f59a23"
          ></uni-icons
          >{{$t('Equipment.InputBindInfo.bind-success')}}</text
        >
      </view>
    </uni-popup>
  </view>
  ...
  methods: {
    ...
    formSubmit(e) {
      ...
      if (this.inputEquipmentCode == "") {
        uni.showToast({
          icon: "none",
          title: this.$t('Equipment.InputBindInfo.input-devices-code-tip'),
        });
        return;
      }
      // 输入的设备名称为空
      if (this.inputEquipmentName == "") {
        uni.showToast({
          icon: "none",
          title: this.$t('Equipment.InputBindInfo.input-devices-name-tip'),
        });
        return;
      }
      uni.request({
        url:
          this.host +"/app/user/product/bind/machine/" +this.selectedProduct +"/" +this.inputEquipmentCode +"/"+this.inputEquipmentName,
        method: "POST",
        header: {
          "x-token": this.userToken,
        },
        success: (res) => {
          if (res.data.code == 0) {
            uni.showToast({
              icon: "none",
              title: this.$t('Equipment.InputBindInfo.bind-success'),
            });
            uni.switchTab({
              url: "/pages/equipment/equipmentMenu",
            });
          } else {
            uni.showToast({
              icon: "none",
              title: this.$t('Equipment.InputBindInfo.bind-fail'),
            });
          }
        },
        complete: (res) => {
