	...
	<view class="content">
		...
		<!-- <view class="mod2 flex-col justify-end" v-if="select">
		  <view class="group1 flex-row justify-between">
		    <text class="info2">{{$t('mine.change-tel-vali')}}</text>
		    <text class="word3">{{$t('mine.change-tel-bind')}}</text>
		  </view>
		...
		<!-- <view class="outer3 flex-col justify-end"v-else>
		  <view class="outer4 flex-row justify-between">
		    <text class="xiugai-4">{{$t('mine.change-tel-vali')}}</text>
		    <text class="info2">{{$t('mine.change-tel-bind')}}</text>
		  </view>
		...
		<view class="header-bg" v-if="select">
			<text>{{ $t('common.setTelTitle') }}</text>
		</view>
		<!-- 原手机号 -->
		<view class="wrap2 flex-col" v-if="select">
		  <!-- <view class="InputNumber1 flex-col">
		    <uni-easyinput :disabled="true" :inputBorder="false" v-model="oldtel" placeholder="请输入手机号"></uni-easyinput>
		  </view>
		  <view class="block1 flex-col">
		    <view class="vali-block">
		    	<view class="telvalicode">
		    		<uni-easyinput :inputBorder="false" v-model="oldvalicode" :placeholder="$t('mine.change-telvericode')"></uni-easyinput>
		    	</view>
		    	<image
		    	  class="icon1"
		...
		<!-- 绑定新手机 -->
		 <view class="bd10 flex-col" v-else>
		  <!-- <view class="InputNumber1 flex-col">
		      <uni-easyinput :inputBorder="false" v-model="newtel" :placeholder="$t('mine.change-newtel')"></uni-easyinput>
		  </view>
		  <view class="block1 flex-col">
		    <view class="vali-block">
		    	<view class="telvalicode">
		    		<uni-easyinput :inputBorder="false" v-model="newvalicode" :placeholder="$t('mine.change-telvericode')"></uni-easyinput>
		    	</view>
		    	<image
		    	  class="icon1"
		    	  referrerpolicy="no-referrer"
		...
		methods: {
			select1() {
				...
				uni.request({
					...
					url: this.host+'/app/checksmscode?type=changephone&value='+this.oldtel+'&code='+this.oldvalicode,
					method: 'GET',
					data: {},
					dataType:'json',
					success: (res) => {
			...
			select2(){
				...
				uni.request({
					...
					url: this.host+'/app/changephone?newphone='+this.newtel+'&code='+this.newvalicode,
					method: 'GET',
					data: {},
					dataType:'json',
					success: async(res) => {
						...
						prevPage.$vm.otherFun2(object)// 給上一頁綁定方法otherFun,傳參object
						uni.navigateBack({
							delta:1
						})
					},
			...
			oldsendvalidate(){
				...
				uni.request({
					...
					url: this.host+'/app/getsmscode?type=changephone&value='+this.oldtel,
					method: 'GET',
					data: {},
					dataType:'json',
					success: (res) => {
			...
			newsendvalidate(){
				...
				uni.request({
					...
					url: this.host+'/app/getsmscode?value='+this.newtel+'&type=newphone',
					method: 'GET',
					data: {},
					dataType:'json',
					success: (res) => {
