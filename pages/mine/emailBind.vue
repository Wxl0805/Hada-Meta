<template>
	<view class="">
		...
		<view class="outer6 flex-col">
		    ...
		    <!-- <view class="mod1 flex-col flex-row" >
		      <view class="email-all">
		    	...
		    	<view class="email-input">
		    		<uni-easyinput :inputBorder="false" v-model="email" :placeholder="$t('mine.inputemail')"></uni-easyinput>
		    	</view>
		      </view>
		    </view> -->
			...
			<view class="input">
				<view class="input-left">Email</view>
		...
		methods: {
			...
			sendvalidate(){
				...
				uni.request({
					...
					url: this.host+'/app/getemailcode?value='+this.email+'&type=bindemail',
					method: 'GET',
					data: {},
					dataType:'json',
					success: (res) => {
			...
			bind(){
				...
				uni.request({
					...
					url: this.host+'/app/bindemail?email='+this.email+'&code='+this.valicode,
					method: 'GET',
					data: {},
					dataType:'json',
					success: (res) => {
						...
						if(res.data.code == 7){
							...
							if(res.data.msg == '此邮箱已被他人注册'){
								uni.showToast({
									title:this.$t('common.binded-email'),
									icon:'none'
								})
								return;
							}
						...
						if(res.data.code == 0){
							...
							prevPage.$vm.bindemail(object)// 給上一頁綁定方法otherFun,傳參object
							uni.navigateBack({
								delta:1
							})
							uni.showToast({
