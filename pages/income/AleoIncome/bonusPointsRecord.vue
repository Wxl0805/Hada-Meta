<template>
  <view class="top">
    ...
    <view class="uni-container" v-if="showTable">
      <view v-for="(item, index) in bonusInfo" :key="index">
...
export default {
  ...
  methods: {
    // 获取接口 用户拥有的獎勵積分記錄
    getUserBonusPointsRecord() {
      uni.request({
        url: this.host + "/app/revenuerecord",
        // url: "http://192.168.3.33:8887/app/revenuerecord",
        method: "GET",
        header: {
          //token
        ...
        success: (res) => {
          ...
          if (res.data.code == 0) {
            ...
            this.bonusInfo = res.data.data.data;
            // 获取总共有多少条数据
  ...
  onReachBottom() {
    ...
    setTimeout(function () {
      if (_this.recordData.length < _this.totalLength) {
        ...
        // 获取接口 用户的投資記錄
        uni.request({
          url: _this.host + "/app/revenuerecord",
          method: "GET",
          data: {
            keyword: 2,
            page: _this.page,
